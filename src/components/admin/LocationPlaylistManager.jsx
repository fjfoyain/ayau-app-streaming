...
import {
  ...
  getAvailablePlaylistsForLocation,
  assignPlaylistToLocation,
  removePlaylistFromLocation,
} from '../../services/supabase-api';

export default function LocationPlaylistManager({ location, open, onClose }) {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const [locationPlaylists, available] = await Promise.all([
        getLocationPlaylists(location.id),
        getAvailablePlaylistsForLocation(location.id),
      ]);

      setAssignedPlaylists(locationPlaylists || []);
  ...
  const handleAssignPlaylist = async () => {
    ...
    setSaving(true);
    try {
      await assignPlaylistToLocation(location.id, selectedPlaylist);
  ...
  const handleRemovePlaylist = async (playlistId) => {
    if (!confirm('¿Remover esta playlist del local?')) return;

    setSaving(true);
    try {
      await removePlaylistFromLocation(location.id, playlistId);
          ...
          <>
            ...
            >
              <Typography variant="body2" sx={{ color: '#2196F3', fontSize: '0.85rem' }}>
                <strong>ℹ️ Nota:</strong> Solo puedes asignar playlists que ya están asignadas a la cuenta de este local.
                Para agregar más playlists, primero debes asignarlas en la gestión de cuentas.
              </Typography>
            </Paper>
            ...
            {assignedPlaylists.length === 0 ? (
              <Typography sx={{ color: '#F4D03F66', mb: 3, fontStyle: 'italic' }}>
                No hay playlists asignadas a este local
              </Typography>
            ) : (
            ...
            {/* Assign New Playlist */}
            <Typography variant="h6" sx={{ color: '#F4D03F', mb: 2 }}>
              Asignar Playlist de la Cuenta
            </Typography>
            {availablePlaylists.length === 0 ? (
              <Typography sx={{ color: '#F4D03F66', mb: 3, fontStyle: 'italic' }}>
                {assignedPlaylists.length > 0
                  ? 'Todas las playlists de la cuenta ya están asignadas a este local'
                  : 'No hay playlists disponibles. Primero asigna playlists a la cuenta.'}
              </Typography>
            ) : (
